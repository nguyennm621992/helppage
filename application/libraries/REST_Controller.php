...
abstract class REST_Controller extends CI_Controller
{
	 ...
	 * @var array
	 */
	protected $allowed_http_methods = array('get', 'delete', 'post', 'put', 'patch');
	...
	*/
	...
	public function response($data = array(), $http_code = null)
	{
		...
		else
		{
			...
			// Added for error handling on browser 14.5.20 shyang
			if(empty($data['success']) && $this->get_param('disable_error_msg') ){
				echo '-1';
				exit;
			}

			/*
			// Added for error handling on browser 14.2.20 by Hanbiro Dev Team
			if( ! $this->input->is_ajax_request() && ! $this->get_param('api'))
			{
				if(empty($data['success']) && ! empty($data['msg']))
				{
					$message = $data['msg'];
					...
					show_error($message, $http_code);
					exit();
				}
			}*/

			if($this->input->is_ajax_request() && $http_code != 200)
			{
				$this->response->format = config_item('rest_default_format');
			}
